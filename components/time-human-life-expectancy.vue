 ...
 * ------------------------------------------------------------------------- */
const getDuration = () => {
  const expectancy = 73.16; // as of 2023
  const start = Math.floor(dayjs().year() * expectancy);
  const end = Math.ceil(dayjs().year() * expectancy);

  const lifeBeginning = dayjs(`${start}-01-01`);
  const lifeEnd = dayjs(`${end}-01-01`);
  return lifeEnd.diff(lifeBeginning);
};
